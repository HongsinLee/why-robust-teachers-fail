...
# ---------------- 1) Arguments ----------------
def parse_args():
    parser = argparse.ArgumentParser(description="Run synthetic AT experiment.")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="List of unlearnable fractions to run, e.g., --p_un_list 0.0 0.05 0.1 0.15 0.2.",
    )
    ...
    # Output
    parser.add_argument("--out_dir", type=str, default="synthetic_exp/results/at")
    parser.add_argument("--save_diagnostic_plot", action="store_true")

    return parser.parse_args()


def make_run_dir(out_dir, p_un, seed):
...
class ReLUNet:
    ...
    def train_attack(self, X, y, eps, u_idx):
        """
        Closed-form training attack along the learnable coordinate.

        Since the student is blind to v, unlearnable samples effectively have
        no usable robust signal for the student.
        """
        if eps == 0:
            return X.copy()
...
def run_single(args, p_un):
    ...
    run_dir = make_run_dir(args.out_dir, p_un, args.seed)
    ...
    print(f"[Run] p_un={p_un:.2f}, seed={args.seed}")
    ...
    for t in tqdm(range(args.steps)):
        ...
        grad = model.grad_weights(X_adv_i, y_i)
        ...
        if t % args.eval_interval == 0 or t == args.steps - 1:
            ...
            loss_all = np.mean(
                [ce_loss(model, X, y) for X, y in zip(X_eval, y_eval)]
            )

            loss_learn = (
                np.mean([ce_loss(model, X, y) for X, y in zip(X_learn, y_learn)])
                if len(X_learn) > 0
                else np.nan
            )
            ...
            loss_unlearn = (
                np.mean(
                    [ce_loss(model, X, y) for X, y in zip(X_unlearn, y_unlearn)]
                )
                if len(X_unlearn) > 0
                else np.nan
            )
    ...
    print(
        f"[Final] robust_train={df['Robust_Train_Acc'].iloc[-1]:.1f}, "
